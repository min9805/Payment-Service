...
class UserDtoTest : BehaviorSpec({
    ...
    Given("UserDtoRequest 를 사용할 때") {
        When("모든 형식이 올바르다면") {
            val rightDto = UserDtoRequest("test@test.com", "Password!123", "name", "nickname", "01012341234")
        ...
        When("이메일 형식이 올바르지 않다면") {
            val emailDto = UserDtoRequest("test", "Password!123", "name", "nickname", "01012341234")
            Then("예외가 발생한다.") {
        ...
        When("비밀번호 형식이 올바르지 않다면") {
            val passwordDto = UserDtoRequest("test@test.com", "Password", "name", "nickname", "01012341234")
            Then("예외가 발생한다.") {
        ...
        When("전화 번호가 11자리가 아니라면") {
            val phone_number_size_Dto = UserDtoRequest("test@test.com", "Password!123", "name", "nickname", "0111")
            Then("예외가 발생한다.") {
        ...
        When("전화번호 형식이 올바르지 않다면") {
            val valid_phone_number_Dto =
                UserDtoRequest("test@test.com", "Password!123", "name", "nickname", "11384728592")
            Then("예외가 발생한다.") {
